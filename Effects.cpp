...
void Effects::start(EffectType effect_type, float effect_speed)
{
    ...
    switch (m_current_effect)
    {
        ...
        case SHAKE:   
            //m_shader_program.set_view_matrix(m_view_offset);
            break;
    }
}

void Effects::start(EffectType effect_type, glm::mat4& view_matrix) {
    m_current_effect = effect_type;
    switch (m_current_effect)
    {
    case SHAKE:
        view_matrix += glm::translate(view_matrix, m_view_offset);
        //m_alpha = 1.0f;
        break;
    default:
        break;
...
void Effects::update(float delta_time)
{
   switch (m_current_effect)
   {
       ...
       case SHRINK:
           ...
           break;
       case SHAKE:
           /* rand val for screenshake */
           float random_float = ((float)rand() / RAND_MAX) * (0.0f - MAX_OFFSET) + MAX_OFFSET;
           m_view_offset = glm::vec3(random_float, random_float, 0);
           break;
   }
}
